...
def custom_sha1(message, h= None, l=None):
    if h:
        assert len(h) == 5
        h0, h1, h2, h3, h4 = h
    else:
...
def pad(l):
    ...
    r = ml + 8
    p += b"\x00" * (((448 - r) % 512) // 8)
    p += int.to_bytes(ml, 8, "big") 
    return p

def hle(text, text_mac):
    hh = int.from_bytes(text_mac, "big")
    ...
    h4 = hh & 0xFFFFFFFF
    for i in range(32 + 1):
        add_text = b";admin=true"
        full_text = text + pad(i + len(text)) + add_text        
        forged_hash = custom_sha1(add_text, [h0, h1, h2, h3, h4], len(full_text) + i)
        if forged_hash == mac(full_text):
            print(f"matched when i is {i}")
            break
    return i

assert hle(text, text_mac) == kl